...
This repository contains a production-quality, personal backend integration bot connecting Discord Gateway and Telegram.

## 🏗️ System Architecture

- **Runtime**: Node.js (v22+), TypeScript (Strict mode), ES Modules (`"type": "module"`).
- **Discord**: `discord.js` v14 (WebSocket Gateway, Intents: `Guilds`, `GuildMembers`).
...
├── commands/     # Slash command handlers (Discord) & Telegram command router
├── formatters/   # Discord Embed and Telegram HTML message formatters
...
3. **No Web UI / Frontend**: Do NOT add React, Next.js, HTML views, customer dashboards, or payment/licensing logic.
4. **Secrets Security**:
...
5. **Discord Permissions**:
   - Never request or enforce `Administrator` permission for the bot.
   - Enforce `Manage Server` check on server side for `/setup` and `/config`.
   - Use minimum permissions (`SendMessages`, `EmbedLinks`).
...
- DO NOT fabricates or claim unavailable Discord member information.
